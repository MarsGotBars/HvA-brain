...
async function getComponentDependencies(componentPath, analyzed = new Set()) {
  ...
  // Debug logging
  console.log(`  🔍 Analyzing component: ${componentPath} (name: ${componentName})`);
  ...
  try {
    ...
    console.log(`    ❌ No CSS found at: ${cssPath}`);
  }
  ...
  try {
    ...
    console.log(`    📄 Found liquid file, scanning for nested renders...`);
    const nestedRenders = content.match(/\{%\s*render\s+['"](.*?)['"][^%]*%\}/g) || [];
    console.log(`    Found ${nestedRenders.length} nested render(s)`);

    for (const match of nestedRenders) {
      const nestedMatch = match.match(/['"](.*?)['"]/);
      if (nestedMatch) {
        const nestedPath = nestedMatch[1];
        console.log(`      ↳ Found nested component: ${nestedPath}`);
    ...
    console.log(`    ❌ No liquid file found at: ${liquidPath}`);
  }

  console.log(`    📦 Returning CSS: [${css.join(", ")}], JS: [${js.join(", ")}]`);
